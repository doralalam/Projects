...
INPUT_PATH = BASE_DIR / "data" / "separated_files" / "nippon"

MASTER_BASE = BASE_DIR / "data" / "master_dataset"
PARQUET_PATH = MASTER_BASE / "parquet_files"
XLSX_PATH = MASTER_BASE / "xlsx_files"

MASTER_PARQUET = PARQUET_PATH / "nippon_amc.parquet"
MASTER_XLSX = XLSX_PATH / "nippon_amc.xlsx"
...
def setup_logging():
    ...
    log_file = LOG_PATH / "nippon_standardizer.log"
    logging.basicConfig(
        level=logging.INFO,
...
def clean_fund_name(fund_name):
    ...
    if not fund_name.startswith("Nippon India"):
        fund_name = "Nippon India " + fund_name
...
def process_file(file_path):
    ...
    df_filtered = df.loc[mask_not_blank & mask_starts_ine & mask_yield_blank].copy()
    mask_not_all_blank = ~(
    ...
    fund_raw = re.sub(r"^Nippon India\s+", "", fund_full)
    fund_raw = re.sub(r"_.*$", "", fund_raw)
    ...
    out = df_filtered.loc[:, [
        stock_col, isin_col, sector_col, quantity_col, market_value_col, weight_col
    ]].copy()
    ...
    out = out.assign(amc="Nippon India", fund=fund, month=month, year=year)
    out = out[["amc", "fund", "stock", "isin", "sector",
               "quantity", "market_value", "weight", "month", "year"]]

    # Clean stock names for consistency
    out['stock'] = out['stock'].apply(clean_stock_name)
    for col in ["isin", "sector", "fund", "amc", "month", "year"]:
...
def main():
    setup_logging()
    logging.info("Starting Nippon standardization")
    process_all_files()
