...
def calculate_fund_conviction(latest, mom, qoq):
    mom_score = max(mom, 0)
    qoq_score = max(qoq, 0)
    conviction = latest * 0.6 + mom_score * 0.2 + qoq_score * 0.2
    return conviction
...
def calculate_amc_conviction(latest, mom, qoq, fund_count):
    mom_score = max(mom, 0)
    qoq_score = max(qoq, 0)
    conviction = latest * 0.5 + mom_score * 0.2 + qoq_score * 0.2 + fund_count * 2
    return conviction
...
# Group by ISIN 
stock_totals = full_df.groupby(['ISIN', 'Stock Name'])[latest_col].sum().reset_index()
stock_totals = stock_totals.sort_values(by=latest_col, ascending=False)

for _, stock_row in stock_totals.iterrows():
    isin = stock_row['ISIN']
    stock_name = stock_row['Stock Name']

    group = full_df[(full_df['ISIN'] == isin) & (full_df['Stock Name'] == stock_name)]

    # Totals for the entire AMC
    ...
    status_total = determine_status(total_latest, total_prev, total_third, mom_total, qoq_total)
    conviction_total = calculate_amc_conviction(total_latest, mom_total, qoq_total, fund_count)
    ...
    for _, row in group_sorted.iterrows():
        ...
        status_fund = determine_status(latest_val, prev_val, third_val, mom, qoq)
        fund_conviction = calculate_fund_conviction(latest_val, mom, qoq)
